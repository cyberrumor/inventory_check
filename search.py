...
if __name__ == '__main__':
	...
	drop_pages = []
	for item in walmart.search(product) + newegg.search(product) + bestbuy.search(product) + amazon.search(product):
	...
	while 1:
		for item in desired_items:
			...
			if item['site'] == 'newegg':
				...
				try:
					r = requests.get(item['url'], headers = amazon_header)
					r.raise_for_status()
				except Exception as e:
					print(f'amazon raised error: {e}')
					wait += 1
					print(f'raising interval to {wait} seconds.')
					continue
				soup = BeautifulSoup(r.text, 'lxml')
				if amazon.item_is_in_stock(soup):
					notify(item)
